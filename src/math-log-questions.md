...
* **Псевдобулева алгебра (алгебра Гейтинга).** Импликативная решетка с 0
  ...
  * 0 - наименьший элемент решетки
  ...
  Поскольку в принятом нами интуиционистком исчислении высказываний доказываются не высказывания, а некоторое сложные условные выражения (записи вида $\Gamma\vdash\alpha$), то и оценкой для данных выражений мы выберем неравенство. А именно, если $\Gamma=\{\gamma_1,...\gamma_n\},g_i=[\gamma_i]\ и\ a=[\alpha]$, то выражению $\Gamma\vdash\alpha$ сопоставим неравенство: $g_1\ast g_2\ast...\ast g_n\sqsubseteq a$.
  ...
  * *Аксиома.*  $G,a\sqsubseteq a$
    ...
    Распишем правило вывода по определению в виде неравентсва: $g_1\ast g_2\ast...\ast g_n\ast a\sqsubseteq a$. Тк операция нижней грани только уменьшает результат, то если $a\sqsubseteq g_1$, то $a\ast g_1=c, c\sqsubseteq a$, но верхней гранью всего произведения является $a$, а по рефлексивности всего множества $a\sqsubseteq a$. Так что неравенство выполнено для всех.
...
* **Булева алгебра.** Псевдобулева алгебра, в которой для любых $a$: $a + (a\rightarrow 0) = 1$. \
  ...
  Правило исключенного третьего истинно в классической модели, но не выводимо в теории доказательств ИИВ.
  </details>
...
* **Алгебра Линденбаума.**  $ (\mathcal{L})$  Множество множеств (классов) факторизованных по отношению эквивалентности. 
...
* **Теорема** Модель Крипке это **алгебра Гейтинга**. (Данный вопрос стоит посмотреть в определении модели Крипке как табличная модель)
  ...
  * TODO() , разве такая модель Крипке не генерит диамант в алгебрах Гейтинга (противоречие с дистрибутивностью)? 
  ...
  * TODO() , теория доказательств в моделях Крипке
...
* **Дизъюнктивность ИИВ.** Если $\vdash\alpha\lor\beta$, то $\vdash\alpha$ или $\vdash\beta$
  ...
  * **Доказательство**
    ...
    1. $[[\alpha\vee\beta]]_\mathcal{L}=1\ \ \ \Rightarrow\ \ \ [[\alpha\vee\beta]]_{\Gamma(\mathcal{L})}$
       ...
       Это выполнено, т. к. $\Gamma(\mathcal{L})$-алгебра Гейтинга. Потому что в $\mathcal{L}$ эта формула истинна, а, значит, доказуема (тк в алгебре Линдебаума формула истинна титт, когда она доказуема), тогда эта формула доказуема (этот переход слабый TODO()) и в $\Gamma(\mathcal{L})$, а в ней любая доказуемая формула оценивается в истину.
...
* В исчислении предикатов к схемам аксиом из исчисления высказываний добавляется две схемы аксиом:
  ...
  * Контр-примеры для схем аксиом (когда нарушается свобода для подстановки $\theta$ в $\psi$)
    1. Возьмеме предметные множество $D$ из 2 элементов и больше\ 
      $\forall x.(\psi)\rightarrow(\psi[x:=\theta])\\$
      $\psi:=\exist a.\neg(a=b);\ x:=b;\ \theta=a\\$
...
* **Теорема о дедукции**
  ...
  (1 часть) $\Gamma\vdash\alpha\rightarrow\beta$, то $\Gamma, \alpha\vdash\beta$ - доказывается также, как в ИВ.

  (2 часть) $\Gamma, \alpha\vdash\beta$, то $\Gamma\vdash\alpha\rightarrow\beta$, при условии, что отстутствуют правила для кванторов по свободным переменным из $\alpha$. Также намечаем план, дописывая слева импликацию и пытаемся доказать $\alpha\rightarrow\delta_i$

  1. Аксиомы $1-10$, $\delta_i=\alpha$, Modus Ponens - возьмем доказательство из т о дедукции в ИВ.
  2. Аксиомы $11,12$.  Свобода для подстановки не нарушается, потому что эту аксиому мы уже использовали в каком-то контексте и этот контекст остался $\Rightarrow$ используем такой же вывод, как для аксиом $1-10$
  3. 2 правило
     1. $\alpha\rightarrow\pi\rightarrow\delta$
     2. $(\alpha\rightarrow\pi\rightarrow\delta)\rightarrow(\alpha\&\pi\rightarrow\delta)$ (по полноте ИВ)
     3. $(\alpha\&\pi\rightarrow\delta)$ (MP)
     4. $(\alpha\&\pi\rightarrow\forall x.\delta)$ (тк есть условие на отсутствие применения правил для кванторов по переменным из $\alpha$ и это правило уже было применено к $\pi$ в прошлом док-ве $\rightarrow$ нет свободных вхождений в левую часть)
     5. $(\alpha\&\pi\rightarrow\forall x.\delta)\rightarrow(\alpha\rightarrow\pi\rightarrow\forall x.\delta)$ (по полноте ИВ, если формулу с квантором обозначить просто за $\psi$)
     6. $\alpha\rightarrow\pi\rightarrow\forall x.\delta$ (MP)
  4. 3 правило
     1. $\alpha\rightarrow\pi\rightarrow\delta$
     2. $\pi\rightarrow\alpha\rightarrow\delta$ (по таблице истинности)
     3. $\exist x.(\pi)\rightarrow\alpha\rightarrow\delta$ (по ограничивающему условию и по выполнению в пред доказательстве)
     4. $\alpha\rightarrow\exist x.(\pi)\rightarrow\delta$ (по таблице истинности, если формулу с квантором обозначить за $\psi$)

  </details>

* *Лемма.* $[\phi[x:=\theta]]=[\phi]^{x:=[\theta]}$

  <details><summary>Доказательство</summary>

  Возпользуемся структурой индукцией по выражению $\phi$

  База: $\phi$ - предикат, тогда из способа вычилления предиката, где $f$ - функция оценки: $[\phi]^{x:=[\theta]} = [P(...,x,...)]^{x:=[\theta]} = f_p(...,f_x,...)^[f_x = f_{\theta}] = f_p(...,f_{\theta})= [P(...,\theta,...)=[\phi[x:=\theta]]$

  Переход: $\phi=\alpha\star\beta$ - рассмотрение случаем схоже. Посмотрим на конъюнкцию: $[\alpha\&\beta]^{x:=[\theta]}=[\alpha]^{x:=[\theta]}\&[\beta]^{x:=[\theta]}=[\alpha[x:=\theta]]\&[\beta[x:=\theta]]=[\alpha[x:=\theta]\&\beta[x:=\theta]]=[(\alpha\&\beta)[x:=\theta]]$

  $\phi=\forall y.\psi$. Покажем, что $[\forall y.\psi]^[x:=([\theta]=\forall y.\psi)[x:=[\theta]]$. Без уменьшения общности можем предположить, что $x$ входит свободно в $\forall y.\psi$ —иначе данная замена не оказывает влияние на вычисление результата, и равенство очевидно. Поскольку $\theta$ свободна для подстановки вместо $x$ в $\phi$, то, значит, $y$ не входит свободно в $\theta$ (иначе свободный $y$ стал бы связанным при подстановке, а подстановки обязательно будут иметь место, раз $x$ входит свободно в $\forall y.\psi$). По предположению индукции, $[\psi]^{ x:=[\theta]} = [\psi[x := \theta]]$. И, более того, [\psi]^{x:=[\theta],y:=v_y} = [\psi[x := θ]]^{y:=v_y} , поскольку вычисление оценки θ не зависит от значения переменной $y$. Значит, оценки полных формул с квантором также будут совпадать
  </details>



* **Теорема.** Если $\Gamma\vdash\alpha$ и $\alpha$ нет кванторов по свободным переменным из $\Gamma$, то $\Gamma\vDash\alpha$
  
  <details><summary>Доказательство</summary>
  
  Индукция по длине доказательства
  
  База: 1-10 аксиома - аналогично д-ву в ИВ. 11-12 аксиомы $\Gamma\vdash\forall x.(\phi)\rightarrow\phi[x:=\theta]$. Давайте оценим $[\forall x.(\phi)\rightarrow\phi[x:=\theta]]$: 
    1. $[\forall x.(\phi)]=Л$ - ok
    2. $[\forall x.(\phi)]=И$, это значит, что $[\phi]^{x:=a}=И$ (при всех возможных $x$ - истина). Рассмотрим $a=[\theta]$. В частности, $[\phi]^{x:=[\theta]}=И$. Значит, $[\phi]^{x:=a}\rightarrow[\phi]^{x:=[\theta]}=И$. Тогда по лемме, доказанной выше, все выполняется.
   
  Индукция: $MP$ -также. 2-3 правила: $\Gamma\vdash\psi\rightarrow\phi$, то $\Gamma\vdash\psi\rightarrow\forall x.\phi$. $x$ не взодит свободно в $\Gamma$ - значит, $[\gamma_i]$ не зависит от $x$. Значит, каково бы ни было значение $x$ $[\gamma_i]=И$ и $[\psi]=k$.
    1. $k=Л$ - ok
    2. $k=И$. $\Gamma\vDash\psi\rightarrow\phi$, те $[\psi\rightarrow\phi]^{x:=a}=И$, потому что ни одна из формул из $\Gamma$ не содержит $x$ свободно (по усл). Оценка $\psi$ не зависит от $x$ (по правилу вывода), поэтому $[\psi]\rightarrow[\phi]^{x:=a}$ и $[\psi] = И$, следовательно $[\phi]^{x:=a}=И$. Значит, $[\forall x.phi]=И$ и $[\psi\rightarrow\forall x.phi]=И$. 
...
* **Структура.** Структурой теории первого порядка мы назовем упорядоченную тройку $\langle D,F,P\rangle$, где $F=\langle F_0,F_1...\rangle$ - списки оценок для 0-местных, 1-местных и т.д. функций, и $P=\langle P_0,P_1...\rangle$ - списки оценок для 0-местных, 1-местных и т.д. предикатов, $D$ - предметное множество.
    ...
    **Некорректная структура.** Возьмем нашу формальную арифметику, только изменим оценку предикатк $=$. Оценим его в ложь. Тк $\vdash a=a$ по лекции, то у нас есть выводимое утверждение, но не общезначимое.
...
* **Формальная арифметика.** (формализация аксиоматики Пеано) формальная арифметика - теория первого порядка, со следующими добавленными нелогическими:
  ...
  * <details><summary>Некоторые определения и теоремы Формальной Арифметики</summary>
    ...
    * $x\leq y:=\exist a.x+a=y$
     
    * $x\le y:=\exist a.x+a=y\&\neg (a=0)$
