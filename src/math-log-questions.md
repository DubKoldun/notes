...
* **Непротиворечивость.** Множество высказываний $\Gamma$ непротиворечиво, если не существует формулы $\alpha$ такой, что $\vdash\alpha$ и $\vdash\neg\alpha$
...
* Правила вывода:
  ...
  $${{(\phi)->(\psi)}\over{(\phi)\rightarrow \forall x(\psi)}}$$ $${{(\psi)->(\phi)}\over{\exists x (\psi)\rightarrow (\phi)}}$$
